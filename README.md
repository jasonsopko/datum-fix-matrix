...
Generated 2026-09-06 19:04 UTC from the live repositories by `generate.py`; the page with color is at `docs/index.html` (GitHub Pages). Bold = merged where that build comes from; plain = a PR or branch exists; missing = no fix; n/a = no gateway.

| Fix | OCEAN | innerhat | CONVOY | FlyTheElephant | MaVeTh | Lazarus |
|---|---|---|---|---|---|---|
| **Payouts: a wrong answer here pays miners short** | | | | | | |
| Full payout coinbase on BLAKE2b jobs (class 4, not the 755-byte class 2) | missing | **merged #17** | #10 open | **on master** | missing | branch blake2b-unsplit-coinbase |
...
| Sigop budget on payout outputs (a big P2PKH split can exceed the block limit) | missing | missing | #10 open | missing | missing | n/a |
...
| Header XOR through memcpy (undefined behavior) | missing | missing | #5 open | missing | missing | n/a |
...
| **Platform** | | | | | | |
| Luke's 64-bit Prime ID, submitblock failure handling, oversized tag, jsonrpc free | missing | missing | **on master** | missing | missing | branch late-coinbaser-convoy |
| Wire-time fixes and macOS build (August 26 batch) | missing | **on master** | **on master** | **on master** | branch mainnet-coinbaser-ui-addr | branch blake2b-unsplit-coinbase |
